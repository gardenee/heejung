...
import type { CheerNoteReaction, CreateCheerNoteReactionInput, CheerNoteReactionStats } from '../models/cheer-note-reaction'
import type { ApiResponse } from '../models/common'
import { CHEER_NOTE_REACTION_TYPES } from '../constants/cheer-note-reaction'

export const createCheerNoteReaction = async (input: CreateCheerNoteReactionInput): Promise<ApiResponse<CheerNoteReaction>> => {
  try {
    ...
    if (!input.type || input.type.trim().length === 0 || !CHEER_NOTE_REACTION_TYPES.includes(input.type as (typeof CHEER_NOTE_REACTION_TYPES)[number])) {
      return {
        data: null,
 ...
 */
export const deleteCheerNoteReaction = async (cheerNoteId: number): Promise<ApiResponse<boolean>> => {
  try {
    const userId = getAnonymousUserId()
 ...
 */
export const getCheerNoteReactionStats = async (cheerNoteId: number): Promise<ApiResponse<CheerNoteReactionStats>> => {
  try {
    const userId = getAnonymousUserId()
 ...
 */
export const getBulkCheerNoteReactionStats = async (cheerNoteIds: number[]): Promise<ApiResponse<CheerNoteReactionStats[]>> => {
  try {
    ...
    if (reactions) {
      reactions.forEach(reaction => {
        ...
        if (stats) {
          stats.reaction_counts[reaction.type] = (stats.reaction_counts[reaction.type] || 0) + 1
 ...
 * 사용자의 리액션 목록을 조회합니다
 */
export const getUserCheerNoteReactions = async (userId?: string): Promise<ApiResponse<CheerNoteReaction[]>> => {
  try {
