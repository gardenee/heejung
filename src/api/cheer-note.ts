...
export const createCheerNote = async (
  input: CreateCheerNoteInput,
): Promise<ApiResponse<CheerNote>> => {
  try {
 ...
 */
export const getCheerNoteById = async (
  id: number,
): Promise<ApiResponse<CheerNote>> => {
  try {
    const { data, error } = await supabase
 ...
 */
export const deleteCheerNote = async (
  id: number,
